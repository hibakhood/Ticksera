...
export default function Services() {
  ...
  return (
    <div className="bg-white dark:bg-dark-bg">
      ...
      {/* Hero */}
      <section className="s-section bg-white dark:bg-slate-900 relative overflow-hidden">
        <ShaderBackground />
        <div className="s-inner max-w-6xl mx-auto text-center relative z-10">
          ...
          <h1 className="font-heading text-slate-900 dark:text-white" style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: '0.75rem' }}>Our Services</h1>
          <p className="text-slate-600 dark:text-slate-300 max-w-2xl mx-auto" style={{ marginBottom: '2rem' }}>
          ...
          {/* Segment toggle */}
          <div className="inline-flex items-center gap-1 p-1 rounded-xl bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/20 dark:border-dark-border shadow-sm">
            {([['all', 'All Clients'], ['personal', 'Personal'], ['business', 'Business']] as [Segment, string][]).map(([v, label]) => (
              <button key={v} onClick={() => setSegment(v)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all
                  ${segment === v ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white'}`}>
      ...
      <section className="s-section bg-slate-50 dark:bg-dark-bg/50">
        <div className="s-inner max-w-6xl mx-auto">
          ...
          <div className="mt-8 rounded-2xl bg-slate-50 dark:bg-slate-950 p-6 flex flex-col lg:flex-row items-start lg:items-center gap-4 border border-slate-200 dark:border-slate-800">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 dark:bg-white/10 flex items-center justify-center flex-shrink-0">
              <Building2 className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
            </div>
            <div className="flex-1">
              <p className="text-sm font-bold text-slate-900 dark:text-white">Business-only support</p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Team-focused services available exclusively to business clients.</p>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {BUSINESS_ONLY.map((b, j) => (
                <span key={j} className="text-[10px] font-semibold px-2 py-1 rounded-md bg-emerald-500/10 dark:bg-white/10 text-emerald-700 dark:text-slate-200 border border-emerald-500/20 dark:border-white/10">{b}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      {/* CTA */}
      <section className="s-section bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800">
        <div className="s-inner max-w-3xl mx-auto text-center">
          <h2 className="font-heading text-slate-900 dark:text-white" style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '0.75rem' }}>Need a Custom Solution?</h2>
          <p className="text-slate-600 dark:text-slate-400" style={{ marginBottom: '2rem' }}>Contact us to discuss a tailored IT support plan for your business, with SLA guarantees, dedicated technicians, and 24/7 monitoring.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link to="/login"><Button size="lg">Get Started Free</Button></Link>
            <Link to="/contact">
              <Button variant="ghost" size="lg" className="text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800">Contact Sales</Button>
            </Link>
          </div>
        </div>
      </section>
